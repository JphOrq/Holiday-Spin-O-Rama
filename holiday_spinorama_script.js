const CONFIG = {
  ...
  newyear: {
    ...
    winnerMessages: ["CHEERS!", "NEW YEAR, NEW WIN!", "BOOM! 🎆", "LUCKY YOU!"],
  },
};
...
function getPrizeAtPointer(rotationAngle, prizeCount) {
  const slice = 360 / prizeCount;
  const wheelAngle = normalizeAngle(360 - rotationAngle);
  return Math.floor(wheelAngle / slice) % prizeCount;
}
...
$("spin").addEventListener("click", () => {
  ...
  const desiredRotation = 360 - (index * slice + slice / 2);
  const currentRotation = normalizeAngle(rotation);
  const target = 360 * 6 + ((desiredRotation - currentRotation + 360) % 360);
  rotation += target;
  wheel.style.transform = `rotate(${rotation}deg)`;
  result.textContent = "🎲 SPINNING...";
  setTimeout(() => {
    const landedIndex = getPrizeAtPointer(rotation, c.prizes.length);
    result.textContent = c.prizes[landedIndex];
